...
export async function moveAttendanceToBoxByTenantUseCase(formData: FormData) {
  ...
  if (!attendanceId || !boxId) {
    redirect("/app/dashboard?error=Movimentacao invalida.");
  }
  ...
  const [detail, settings, boxes] = await Promise.all([
