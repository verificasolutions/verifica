...
import { getTenantEmployeeByIdAdmin, setTenantEmployeeActiveStateAdmin } from "@/backend/repos/employees-admin-repo";
import { setTenantUserActiveStateAdmin } from "@/backend/repos/tenant-users-admin-repo";

export async function setTenantEmployeeStateByAdminUseCase(formData: FormData) {
  ...
  const error = await setTenantEmployeeActiveStateAdmin({
    tenantId,
    employeeId,
    isActive,
  });
  ...
  await createAuditLogAdmin({
    ...
    tenant_id: tenantId,
    action: isActive ? "tenant.employee.activated" : "tenant.employee.deactivated",
    entity_type: "employee",
    entity_id: employeeId,
    message: `${admin.email ?? "admin"} ${isActive ? "reativou" : "desativou"} o usuário ${employee.name}.`,
  });
}
