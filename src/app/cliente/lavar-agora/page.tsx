...
import { submitOrderAction, confirmOrderAction } from "./actions";
...
export default async function LavarAgoraPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <main className="relative isolate mx-auto min-h-[100dvh] w-full max-w-md space-y-4 overflow-hidden px-4 py-6">{site?.landing?.cover_image_url ? <div aria-hidden="true" className="customer-portal-banner pointer-events-none fixed inset-0 -z-10 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(rgba(13,17,23,.76),rgba(13,17,23,.94)),url(${JSON.stringify(site.landing.cover_image_url)})` }} /> : null}<Link href="/verifica/cliente/portal" className="text-sm font-medium text-[color:var(--text-secondary)]">← Voltar</Link><p className="text-xs uppercase tracking-[0.22em] text-[color:var(--accent)]">{site?.singleSource.displayName ?? customer.tenantSlug ?? "Portal do cliente"}</p><h1 className="text-2xl font-bold text-[color:var(--text-primary)]">Lavar agora</h1><p className="text-sm text-[color:var(--text-muted)]">Escolha os serviços. O resumo e o total ficam abaixo da seleção.</p>{params.error ? <FlashNotice error={String(params.error)} /> : null}<form action={submitOrderAction} className="space-y-4"><input type="hidden" name="vehicle" value={vehicleId} /><section className="rounded-3xl border border-[color:var(--surface-border)] bg-[color:var(--card)] p-4"><p className="text-sm font-medium text-[color:var(--text-secondary)]">Veículo</p><p className="mt-1 text-sm text-[color:var(--text-primary)]">{vehicle?.plate} — {[vehicle?.brand, vehicle?.model].filter(Boolean).join(" ") || "Veículo"}</p></section><ServiceSelection services={priced} selectedIds={selectedIds} /><SubmitButton pendingLabel="Confirmando...">Dar OK — confirmar contratação</SubmitButton></form></main>;
}
