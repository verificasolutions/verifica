...
import { createAppointmentDraftAction, confirmAppointmentAction } from "./actions";
...
export default async function AgendarPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const scheduledFor = String(params.scheduled_for ?? "");
  ...
  return (
    <main className="mx-auto w-full max-w-md space-y-4 px-4 py-6">
      ...
      <h1 className="text-2xl font-bold text-[color:var(--text-primary)]">Agendar</h1>
        ...
        <section className="space-y-4 rounded-3xl border border-[color:var(--surface-border)] bg-[color:var(--card)] p-4">
          {preview?.error || !preview?.data ? (
            ...
            <>
              ...
              <form action={confirmAppointmentAction} className="space-y-2">
              ...
              <p className="text-center text-xs text-[color:var(--text-soft)]">
                Disponibilidade, preço e duração são validados pelo servidor na confirmação.
              </p>
            </>
          )}
        </section>
      ) : (
        <form action={createAppointmentDraftAction} className="space-y-4">
          ...
          <SubmitButton>Ver resumo do agendamento</SubmitButton>
        </form>
      )}
    </main>
  );
